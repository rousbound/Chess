...
Apart from the main structure, it should be mentioned the other modules such as:

\begin{enumerate}[label=\alph*)]
\item The \textbf{utils} module is in charge of holding all move format
    conversion functions.

    Ex: UCI to algebric, tuple to UCI, etc. (Tuple I refer as the internal
    program representation that was made in this project)

    \item The \textbf{colors} module is in charge of holding colors and color
    conversion for the GUI module.

    \item The \textbf{main} module is in charge of receiving a parameter and
...
command at top level directory ``mychess''.
\begin{lstlisting}
